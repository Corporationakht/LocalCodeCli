...
from smoke.lib.child_process import cmd_lcc_init, cmd_local_code_cli_serve
...
def test_lcc_init_scaffolds_user_config(
    ...
    assert (tmp_path / ".lcc" / ".env").is_file()


def test_local_code_cli_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
        command=cmd_local_code_cli_serve(),
...
def test_claude_cli_prompt_when_available(
    ...
    with start_server(
        ...
        result = subprocess.run(
            [claude_bin, "-p", "Reply with exactly LCC_SMOKE_PONG"],
            cwd=tmp_path,
            env=env,
            capture_output=True,
            text=True,
    ...
    if "LCC_SMOKE_PONG" not in result.stdout:
