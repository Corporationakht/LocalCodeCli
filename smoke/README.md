...
The second command skips everything unless `LCC_LIVE_SMOKE=1` is set, but still
writes skip entries to `.smoke-results/`.

## Product Smoke Run

```powershell
$env:LCC_LIVE_SMOKE = "1"
uv run pytest smoke -n 0 -s --tb=short
```
...
```powershell
$env:LCC_LIVE_SMOKE = "1"
$env:LCC_SMOKE_TARGETS = "providers"
...
catalog/docs and can be overridden with `LCC_SMOKE_MODEL_<PROVIDER>`, for example
`LCC_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
configured, live product smoke fails as `missing_env` unless you explicitly set
`LCC_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `cli` | `lcc-init`, server entrypoint, Claude CLI adaptive thinking, session cleanup | Claude CLI binary and provider only for real CLI |
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `LCC_SMOKE_MODEL_*` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `LCC_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:LCC_LIVE_SMOKE = "1"
$env:LCC_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LCC_LIVE_SMOKE = "1"
$env:LCC_SMOKE_TARGETS = "ollama"
...
```powershell
$env:LCC_LIVE_SMOKE = "1"
$env:LCC_SMOKE_TARGETS = "telegram,discord,voice"
$env:LCC_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LCC_LIVE_SMOKE = "1"
$env:LCC_SMOKE_TARGETS = "nvidia_nim_cli"
$env:LCC_SMOKE_NIM_MODELS = "z-ai/glm-5.1,moonshotai/kimi-k2.6,minimaxai/minimax-m2.7,nvidia/nemotron-3-super-120b-a12b,deepseek-ai/deepseek-v4-pro,deepseek-ai/deepseek-v4-flash"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LCC_LIVE_SMOKE = "1"
$env:LCC_SMOKE_TARGETS = "openrouter_free_cli"
$env:LCC_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,openai/gpt-oss-120b:free,poolside/laguna-m.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LCC_LIVE_SMOKE = "1"
$env:LCC_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```

## Environment

- `LCC_ENV_FILE`: explicit dotenv path for startup/config scenarios.
- `LCC_LIVE_SMOKE=1`: enables live smoke execution.
- `LCC_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `LCC_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `LCC_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `LCC_SMOKE_MODEL_NVIDIA_NIM`, `LCC_SMOKE_MODEL_OPEN_ROUTER`,
  `LCC_SMOKE_MODEL_MISTRAL`, `LCC_SMOKE_MODEL_MISTRAL_CODESTRAL`,
  `LCC_SMOKE_MODEL_DEEPSEEK`, `LCC_SMOKE_MODEL_KIMI`,
  `LCC_SMOKE_MODEL_WAFER`, `LCC_SMOKE_MODEL_OPENCODE`, `LCC_SMOKE_MODEL_OPENCODE_GO`,
  `LCC_SMOKE_MODEL_ZAI`, `LCC_SMOKE_MODEL_FIREWORKS`,   `LCC_SMOKE_MODEL_GEMINI`,
  `LCC_SMOKE_MODEL_GROQ`, `LCC_SMOKE_MODEL_CEREBRAS`,
  `LCC_SMOKE_MODEL_LMSTUDIO`,
  `LCC_SMOKE_MODEL_LLAMACPP`, `LCC_SMOKE_MODEL_OLLAMA`: optional per-provider
  smoke model overrides. Values may include the provider prefix or just the model
  name for that provider.
- `LCC_SMOKE_NIM_MODELS`: optional comma-separated NVIDIA NIM CLI matrix models
  that replace the default characterization set.
- `LCC_SMOKE_NIM_EXTRA_MODELS`: optional comma-separated NVIDIA NIM CLI matrix
  models appended to the default or replacement set.
- `LCC_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `LCC_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `LCC_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `LCC_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `LCC_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `LCC_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `LCC_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `LCC_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
- `target_disabled`: skipped because `LCC_SMOKE_TARGETS` intentionally selected
  a different target.
...
explicitly selected a provider in `LCC_SMOKE_PROVIDER_MATRIX`;
